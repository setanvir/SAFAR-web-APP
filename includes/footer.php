    </main>
    <footer class="footer">
        <div class="container footer-content">
            <div class="footer-brand">
                <h3>SAFAR</h3>
                <p>Your premium travel marketplace. Discover, compare, and book the best tour packages globally.</p>
            </div>
            <div class="footer-links">
                <h4>Quick Links</h4>
                <ul>
                    <li><a href="<?php echo BASE_URL; ?>/pages/index.php">Home</a></li>
                    <li><a href="<?php echo BASE_URL; ?>/pages/login.php">Login</a></li>
                    <li><a href="<?php echo BASE_URL; ?>/pages/signup.php">Register</a></li>
                </ul>
            </div>
            <div class="footer-contact">
                <h4>Contact Us</h4>
                <p>Email: support@safar.com</p>
                <p>Phone: +1 234 567 890</p>
            </div>
        </div>
        <div class="footer-bottom">
            <p>&copy; <?php echo date("Y"); ?> SAFAR Travel Booking. All rights reserved.</p>
        </div>
    </footer>
    <script src="<?php echo BASE_URL; ?>/assets/js/main.js"></script>
</body>
</html>
